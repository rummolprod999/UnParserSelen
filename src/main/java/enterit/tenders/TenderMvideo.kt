...
class TenderMvideo(val tn: DTGetProcListResponse.Tenders.Item, val comp: DTGetProcListResponse.Header.Company) :
    ...
    override fun parsing() {
        ...
        DriverManager.getConnection(UrlConnect, UserDb, PassDb).use(fun(con: Connection) {
            val stmt0 =
                con.prepareStatement("SELECT id_tender FROM ${Prefix}tender WHERE purchase_number = ? AND doc_publish_date = ? AND type_fz = ? AND date_version = ?")
                    .apply {
                        setString(1, tn.id)
