...
import enterit.parsers.ParserOrelStroy
...
class TenderOrelStroy(
    val tn: SafmargT<String>,
    val driver: ChromeDriver,
) : TenderAbstract(),
    ITender {
    init {
        etpName = "Электронная торговая площадка ПАО \"Орелстрой\""
        etpUrl = "https://etzp.orelstroy.ru/"
    }

    override fun parsing() {
        // driver.close()
        driver.get(tn.href)
        val wait = WebDriverWait(driver, ParserOrelStroy.timeoutB)
        ...
        try {
            wait.until(
                ExpectedConditions.visibilityOfElementLocated(
                    By.xpath(
                        "//td[contains(preceding-sibling::td, 'Начало приема предложений')]//div[contains(@class, 'translate-text-')]",
                    ),
                ),
            )
        } catch (e: Exception) {
        ...
        var endDateT =
            driver
                .findElementWithoutException(
                    By.xpath(
                        "//td[contains(preceding-sibling::td, 'Окончание приема предложений')]//div[contains(@class, 'translate-text-')]",
                    ),
                )?.text
                ?.trim {
                    it <=
        ...
        val status =
            driver
                .findElementWithoutException(
                    By.xpath("//td[contains(preceding-sibling::td, 'Статус торгов')]//div[contains(@class, 'translate-text-')]"),
                )?.text
                ?.trim {
                    it <=
                        ' '
        ...
        DriverManager.getConnection(UrlConnect, UserDb, PassDb).use(
            fun(con: Connection) {
                ...
                val fullnameOrg =
                    driver
                        .findElementWithoutException(
                            By.xpath("//td[contains(preceding-sibling::td, 'Наименование')]//div[contains(@class, 'translate-text-')]"),
                        )?.text
                        ?.trim {
                            it <=
                                ' '
                    ...
                    if (rso.next()) {
                        ...
                        val email =
                            driver
                                .findElementWithoutException(
                                    By.xpath(
                                        "//td[contains(preceding-sibling::td, 'Адрес электронной почты')]//div[contains(@class, 'translate-text-')]",
                                    ),
                                )?.text
                                ?.trim {
                                    it <=
                        ...
                        val phone =
                            driver
                                .findElementWithoutException(
                                    By.xpath(
                                        "//td[contains(preceding-sibling::td, 'Номер контактного телефона')]//div[contains(@class, 'translate-text-')]",
                                    ),
                                )?.text
                                ?.trim {
                                    it <=
                        ...
                        val contactPerson =
                            driver
                                .findElementWithoutException(
                                    By.xpath(
                                        "//td[contains(preceding-sibling::td, 'Контактное лицо')]//div[contains(@class, 'translate-text-')]",
                                    ),
                                )?.text
                                ?.trim {
                                    it <=
                ...
                val placingWayName =
                    driver
                        .findElementWithoutException(
                            By.xpath(
                                "//td[contains(preceding-sibling::td, 'Форма проведения торгов')]//div[contains(@class, 'translate-text-')]",
                            ),
                        )?.text
                        ?.trim {
                            it <=
                ...
                val purObj1 =
                    driver
                        .findElementWithoutException(
                            By.xpath("//td[contains(preceding-sibling::td, 'Предмет торгов')]//div[contains(@class, 'translate-text-')]"),
                        )?.text
                        ?.trim {
                            it <=
                                ' '
                ...
                val purObj2 =
                    driver
                        .findElementWithoutException(
                            By.xpath(
                                "//td[contains(preceding-sibling::td, 'Описание предмета договора')]//div[contains(@class, 'translate-text-')]",
                            ),
                        )?.text
                        ?.trim {
                            it <=
                ...
                val currency =
                    driver
                        .findElementWithoutException(
                            By.xpath("//td[contains(preceding-sibling::td, 'Валюта')]//div[contains(@class, 'translate-text-')]"),
                        )?.text
                        ?.trim {
                            it <=
                                ' '
                ...
                val rec1 =
                    driver
                        .findElementWithoutException(
                            By.xpath(
                                "//td[contains(preceding-sibling::td, 'Требования к участникам торгов')]//div[contains(@class, 'translate-text-')]",
                            ),
                        )?.text
                        ?.trim {
                            it <=
                ...
                val rec2 =
                    driver
                        .findElementWithoutException(
                            By.xpath(
                                "//td[contains(preceding-sibling::td, 'Перечень представляемых участниками торгов документов и требования к их оформлению')]//div[contains(@class, 'translate-text-')]",
                            ),
                        )?.text
                        ?.trim {
                            it <=
                ...
                val delivPlace1 =
                    driver
                        .findElementWithoutException(
                            By.xpath(
                                "//td[contains(preceding-sibling::td, 'Регион поставки товаров, выполнения работ, оказания услуг')]//div[contains(@class, 'translate-text-')]",
                            ),
                        )?.text
                        ?.trim {
                            it <=
                ...
                val delivPlace2 =
                    driver
                        .findElementWithoutException(
                            By.xpath(
                                "//td[contains(preceding-sibling::td, 'Место (адрес) поставки товаров, выполнения работ, оказания услуг')]//div[contains(@class, 'translate-text-')]",
                            ),
                        )?.text
                        ?.trim {
                            it <=
                ...
                var delivTerm1 =
                    driver
                        .findElementWithoutException(
                            By.xpath(
                                "//td[contains(preceding-sibling::td, 'Условия поставки товаров')]//div[contains(@class, 'translate-text-')]",
                            ),
                        )?.text
                        ?.trim {
                            it <=
                ...
                var delivTerm2 =
                    driver
                        .findElementWithoutException(
                            By.xpath(
                                "//td[contains(preceding-sibling::td, 'Пояснение условий поставки')]//div[contains(@class, 'translate-text-')]",
                            ),
                        )?.text
                        ?.trim {
                            it <=
                ...
                val purobj1 =
                    driver.findElements(
                        By.xpath(
                            "//table[preceding-sibling::div[contains(., 'Позиции спецификации:')] and  contains(@class, 'competitive-table-data')]//tr[position() > 1]",
                        ),
                    )
                purobj1.forEach {
                    val name =
