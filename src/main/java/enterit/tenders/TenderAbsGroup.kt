...
class TenderAbsGroup(
    val tn: SafmargT<String>,
    val driver: ChromeDriver,
) : TenderAbstract(),
    ITender {
    init {
        etpName = "АБСОЛЮТ ИНВЕСТИЦИОННАЯ ГРУППА"
        etpUrl = "https://tender.absgroup.ru/"
    }
        ...
        DriverManager.getConnection(UrlConnect, UserDb, PassDb).use(
            fun(con: Connection) {
                ...
                val fullnameOrg =
                    driver
                        .findElementWithoutException(
                            By.xpath("//div[. = 'Организатор']/following-sibling::div/div"),
                        )?.text
                        ?.trim { it <= ' ' }
                        ?: ""
                if (fullnameOrg != "") {
                    ...
                    if (rso.next()) {
                        ...
                        val email =
                            driver
                                .findElementWithoutException(
                                    By.xpath("//div[. = 'Адрес электронной почты']/following-sibling::div/div"),
                                )?.text
                                ?.trim {
                                    it <=
                                        ' '
                        ...
                        val phone =
                            driver
                                .findElementWithoutException(
                                    By.xpath("//div[. = 'Номер контактного телефона']/following-sibling::div/div"),
                                )?.text
                                ?.trim {
                                    it <=
                                        ' '
                        ...
                        val contactPerson =
                            driver
                                .findElementWithoutException(
                                    By.xpath("//div[. = 'Контактное лицо']/following-sibling::div/div"),
                                )?.text
                                ?.trim {
                                    it <=
                                        ' '
                ...
                var idTender = 0
                val placingWayName =
                    driver.findElementWithoutException(By.xpath("//div[. = 'Способ закупки']/following-sibling::div/div"))?.text?.trim {
                        it <=
                            ' '
                    }
                        ?: ""
                ...
                val nmck =
                    driver
                        .findElementWithoutException(By.xpath("//div[. = 'Начальная стоимость']/following-sibling::div/div"))
                        ?.text
                        ?.trim {
                            it <=
                                ' '
                ...
                val rec1 =
                    driver
                        .findElementWithoutException(
                            By.xpath("//div[. = 'Требования к участникам']/following-sibling::div/div"),
                        )?.text
                        ?.trim {
                            it <=
                                ' '
                ...
                val rec2 =
                    driver
                        .findElementWithoutException(
                            By.xpath(
                                "//div[. = 'Перечень представляемых участниками закупки документов и требования к их оформлению']/following-sibling::div/div",
                            ),
                        )?.text
                        ?.trim {
                            it <=
                ...
                val delivPlace1 =
                    driver
                        .findElementWithoutException(
                            By.xpath("//div[. = 'Регион поставки, выполнения работ']/following-sibling::div/div"),
                        )?.text
                        ?.trim {
                            it <=
                                ' '
                ...
                var delivTerm1 =
                    driver.findElementWithoutException(By.xpath("//div[. = 'Условия оплаты']/following-sibling::div/div"))?.text?.trim {
                        it <=
                            ' '
                    }
                        ?: ""
