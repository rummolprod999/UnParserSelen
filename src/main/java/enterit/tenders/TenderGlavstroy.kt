...
import enterit.parsers.ParserGlavstroy
...
class TenderGlavstroy(val tn: SafmargT<String>, val driver: ChromeDriver, val wait: WebDriverWait) : TenderAbstract(),
    ITender {
    ...
    companion object TypeFz {
        const val typeFz = 393
    }

    init {
        etpName = "Тендерная площадка \"Главстрой\""
        etpUrl = "https://tender.glavstroy.ru/"
    }

    override fun parsing() {
        driver.get(tn.href)
        val wait = WebDriverWait(driver, ParserGlavstroy.timeoutB)
        Thread.sleep(5000)
        try {
        ...
        DriverManager.getConnection(UrlConnect, UserDb, PassDb).use(fun(con: Connection) {
            ...
            var scoringDateT =
                driver.findElementWithoutException(By.xpath("//div[contains(preceding-sibling::div, 'Дата подведения итогов')]/div"))?.text?.trim { it <= ' ' }
                    ?: ""
            ...
            lots.forEachIndexed { index, lot ->
                ...
                purobj1.forEach {
                    val test = it.findElementWithoutException(By.xpath(".//td[7]"))?.text?.trim { it <= ' ' }
                        ?: ""
                    if (test != "") {
