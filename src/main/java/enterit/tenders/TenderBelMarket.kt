        ...
        DriverManager.getConnection(UrlConnect, UserDb, PassDb).use(fun(con: Connection) {
            ...
            purobj1.forEach {
                ...
                val quantity = it.selectFirst("td:eq(4)")?.ownText()?.replace(',', '.')?.trim { it <= ' ' }
                        ?: ""
                val price = it.selectFirst("td:eq(5) p")?.ownText()?.replace("&nbsp;", "")?.replace(',', '.')?.deleteAllWhiteSpace()?.trim { it <= ' ' }
                        ?: ""
                val sum = it.selectFirst("td:eq(6) p")?.ownText()?.replace("&nbsp;", "")?.replace(',', '.')?.deleteAllWhiteSpace()?.trim { it <= ' ' }
                        ?: ""
