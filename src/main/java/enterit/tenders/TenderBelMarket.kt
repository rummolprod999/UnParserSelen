...
class TenderBelMarket(
    ...
    init {
        etpName = "ЭЛЕКТРОННЫЙ МАРКЕТ (МАГАЗИН) БЕЛГОРОДСКОЙ ОБЛАСТИ ДЛЯ \"МАЛЫХ ЗАКУПОК\""
        etpUrl = "https://belgorodmarket-app.rts-tender.ru/"
    }
        ...
        DriverManager.getConnection(UrlConnect, UserDb, PassDb).use(fun(con: Connection) {
            ...
            purobj1.forEach {
                ...
                val quantity = it.selectFirst("td:eq(4)")?.ownText()?.replace(',', '.')?.trim { it <= ' ' }
                    ?: ""
                val price = it.selectFirst("td:eq(5) p")?.ownText()?.replace("&nbsp;", "")?.replace(',', '.')
                    ?.deleteAllWhiteSpace()?.trim { it <= ' ' }
                    ?: ""
                val sum = it.selectFirst("td:eq(6) p")?.ownText()?.replace("&nbsp;", "")?.replace(',', '.')
                    ?.deleteAllWhiteSpace()?.trim { it <= ' ' }
                    ?: ""
                val fullOkpd = it.selectFirst("td:eq(2)")?.ownText()?.replace(',', '.')?.trim { it <= ' ' }
                    ?: ""
                val okpd2 = fullOkpd.regExpTest("^(.+)\\s+/")
                val okpdName = fullOkpd.regExpTest("/\\s*(.*)\$")
                con.prepareStatement("INSERT INTO ${Prefix}purchase_object SET id_lot = ?, id_customer = ?, name = ?, okei = ?, quantity_value = ?, customer_quantity_value = ?, price = ?, sum = ?, okpd2_code = ?, okpd_name = ?")
                    .apply {
                        setInt(1, idLot)
                        setInt(2, idCustomer)
                        setString(3, name)
