...
import enterit.parsers.ParserMedsi
...
class TenderMedsi(
    ...
    ITender {
    init {
        etpName = "Медицинская корпорация МЕДСИ"
        etpUrl = "https://medsi.ru/"
    }

    override fun parsing() {
        // driver.close()
        driver.get(tn.href)
        val wait = WebDriverWait(driver, ParserMedsi.timeoutB)
        ...
        try {
            wait.until(
                ExpectedConditions.visibilityOfElementLocated(
                    By.xpath(
                        "//div[contains(preceding-sibling::div, 'Начало представления предложений') or contains(preceding-sibling::div, 'Начало приема квалификационных документов')]",
                    ),
                ),
            )
        } catch (e: Exception) {
        ...
        DriverManager.getConnection(UrlConnect, UserDb, PassDb).use(
            fun(con: Connection) {
                ...
                var delivTerm2 =
                    driver
                        .findElementWithoutException(
                            By.xpath("//div[contains(preceding-sibling::div, 'Пояснение условий поставки')]"),
                        )?.text
                        ?.trim {
                            it <=
                                ' '
                ...
                purobj1.forEach {
                    ...
                    if (test2 != "" && test == "") {
                        ...
                        val quantity =
                            it
                                .findElementWithoutException(By.xpath(".//td[3]"))
                                ?.text
                                ?.trim { it <= ' ' }
                                ?.replace(",", "")
                                ?.deleteAllWhiteSpace()
                    ...
                    val delivPlace =
                        it.findElementWithoutException(By.xpath(".//td[7]"))?.text?.trim { it <= ' ' }
                            ?: ""
                    val delivTermT =
                        it.findElementWithoutException(By.xpath(".//td[8]"))?.text?.trim { it <= ' ' }
                            ?: ""
                    val delivTerm =
                        if (delivTermT != "") {
