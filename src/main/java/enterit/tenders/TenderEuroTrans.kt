    ...
    override fun parsing() {
        ...
        var datePubT =
            driver
                .findElementWithoutException(
                    By.xpath("//div[contains(., 'Начало представления предложений (этап №1)')]/following-sibling::div/div"),
                )?.text
                ?.trim {
                    it <=
                        ' '
        ...
        if (datePubT == "") {
            datePubT =
                driver
                    .findElementWithoutException(
                        By.xpath("//div[. = 'Начало подтверждения участия']/following-sibling::div/div"),
                    )?.text
                    ?.trim {
                        it <=
                            ' '
        ...
        pubDate = datePubTT.getDateFromString(formatterGpn)
        var endDateT =
            driver
                .findElementWithoutException(
                    By.xpath("//div[contains(., 'Окончание представления предложений')]/following-sibling::div/div"),
                )?.text
                ?.trim {
                    it <=
                        ' '
        ...
        endDate = endDateTT.getDateFromString(formatterGpn)
        ...
        DriverManager.getConnection(UrlConnect, UserDb, PassDb).use(
            fun(con: Connection) {
                ...
                if (fullnameOrg != "") {
                    ...
                    if (rso.next()) {
                        ...
                        val phone =
                            driver
                                .findElementWithoutException(
                                    By.xpath("//div[contains(., 'Адрес электронной почты')]/following-sibling::div/div"),
                                )?.text
                                ?.trim {
                                    it <=
                                        ' '
                        ...
                        val contactPerson =
                            driver
                                .findElementWithoutException(
                                    By.xpath("//div[contains(., 'Контактное лицо')]/following-sibling::div/div"),
                                )?.text
                                ?.trim {
                                    it <=
                                        ' '
                ...
                val placingWayName =
                    driver
                        .findElementWithoutException(
                            By.xpath("//div[contains(., 'Способ закупки')]/following-sibling::div"),
                        )?.text
                        ?.trim {
                            it <=
                                ' '
                ...
                val purObj1 =
                    driver
                        .findElementWithoutException(
                            By.xpath("//div[@class = 'mat-tooltip-trigger title flex-um']"),
                        )?.text
                        ?.trim { it <= ' ' }
                        ?: ""
                val purObj2 =
                    driver
                        .findElementWithoutException(
                            By.xpath("//div[contains(., 'Краткое описание предмета договора')]/following-sibling::div/div"),
                        )?.text
                        ?.trim {
                            it <=
                                ' '
                ...
                val LotNumber = 1
                val currency =
                    driver.findElementWithoutException(By.xpath("//div[contains(., 'Валюта')]/following-sibling::div/div"))?.text?.trim {
                        it <=
                            ' '
                    }
                        ?: ""
                ...
                val rec2 =
                    driver
                        .findElementWithoutException(
                            By.xpath(
                                "//div[contains(., 'Перечень представляемых участниками закупки документов и требования к их оформлению')]/following-sibling::div/div",
                            ),
                        )?.text
                        ?.trim {
                            it <=
                ...
                val delivPlace1 =
                    driver
                        .findElementWithoutException(
                            By.xpath("//div[contains(., 'Регион')]/following-sibling::div"),
                        )?.text
                        ?.trim { it <= ' ' }
                        ?: ""
                val delivPlace2 =
                ...
                var delivTerm2 =
                    driver
                        .findElementWithoutException(
                            By.xpath("//div[contains(., 'Отсрочка платежа')]/following-sibling::div/div"),
                        )?.text
                        ?.trim {
                            it <=
                                ' '
                ...
                var delivTerm3 =
                    driver
                        .findElementWithoutException(
                            By.xpath("//div[contains(., 'Сроки поставки')]/following-sibling::div/div"),
                        )?.text
                        ?.trim {
                            it <=
                                ' '
                ...
                var delivTerm4 =
                    driver
                        .findElementWithoutException(
                            By.xpath("//div[contains(., 'Проект')]/following-sibling::div"),
                        )?.text
                        ?.trim { it <= ' ' }
                        ?: ""
