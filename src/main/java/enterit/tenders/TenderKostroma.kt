...
class TenderKostroma(
    ...
    init {
        etpName = "ЭЛЕКТРОННЫЙ МАГАЗИН КОСТРОМСКОЙ ОБЛАСТИ"
        etpUrl = "https://kostromarket.rts-tender.ru/"
    }
    ...
    override fun parsing() {
        ...
        DriverManager.getConnection(UrlConnect, UserDb, PassDb).use(
            fun(con: Connection) {
                ...
                val idPlacingWay = 0
                var idTender = 0
                val idRegion = getIdRegion(con, "костром")
                val insertTender =
                ...
                val delivPlace =
                    htmlTen.selectFirst("td:contains(Место поставки) + td")?.ownText()?.trim { it <= ' ' }
                        ?: ""
                val delivTerm =
                    htmlTen.selectFirst("td:contains(Сроки поставки) + td")?.ownText()?.trim { it <= ' ' }
                        ?: ""
                ...
                purobj1.forEach {
                    val name =
                        it.selectFirst("td:eq(1)")?.text()?.trim { it <= ' ' }
                            ?: ""
                    val s = it.selectFirst("td:eq(1)")
                    val p = it.selectFirst("td:eq(0)")
                    val okei =
                        it.selectFirst("td:eq(3)")?.ownText()?.trim { it <= ' ' }
                            ?: ""
                    val quantity =
                        it
                            .selectFirst("td:eq(4)")
