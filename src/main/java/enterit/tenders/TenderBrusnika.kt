...
import enterit.parsers.ParserBrusnika
...
class TenderBrusnika(val tn: SafmargT<String>, val driver: ChromeDriver) : TenderAbstract(), ITender {
    init {
        etpName = "Электронная торговая площадка БРУСНИКА.SRM"
        etpUrl = "https://srm.brusnika.ru/"
    }

    companion object TypeFz {
        const val typeFz = 346
    }

    override fun parsing() {
        driver.get(tn.href)
        val wait = WebDriverWait(driver, ParserBrusnika.timeoutB)
        Thread.sleep(10000)
        try {
