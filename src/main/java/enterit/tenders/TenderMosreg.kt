        ...
        DriverManager.getConnection(UrlConnect, UserDb, PassDb).use(fun(con: Connection) {
            ...
            if (fullnameOrg != "") {
                ...
                if (rso.next()) {
                    ...
                    stmto.close()
                    val postalAdr =
                        htmlTen.selectFirst("span:contains(Адрес места нахождения:) + p")?.ownText()?.trim { it <= ' ' }
                            ?: ""
                    val factAdr = ""
            ...
            purobj1.forEach {
                ...
                val okei = it.selectFirst("div.centerPart > div > p:eq(0)")?.ownText()?.trim { it <= ' ' }
                    ?: ""
                val quantity =
                    it.selectFirst("div.centerPart > div > p:eq(1)")?.ownText()?.replace(',', '.')?.trim { it <= ' ' }
                        ?: ""
                val price =
                    it.selectFirst("div.rightPart > div > p:eq(0)")?.ownText()?.replace(',', '.')?.trim { it <= ' ' }
                        ?: ""
                val sum =
                    it.selectFirst("div.rightPart > div > p:eq(1)")?.ownText()?.replace(',', '.')?.trim { it <= ' ' }
                        ?: ""
