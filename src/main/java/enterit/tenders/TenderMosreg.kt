    ...
    override fun parsing() {
        ...
        DriverManager.getConnection(UrlConnect, UserDb, PassDb).use(
            fun(con: Connection) {
                ...
                var inn = ""
                val fullnameOrg =
                    htmlTen.selectFirst("span:contains(Полное наименование:) + a")?.ownText()?.trim { it <= ' ' }
                        ?: ""
                if (fullnameOrg != "") {
                    ...
                    if (rso.next()) {
                        ...
                        val postalAdr =
                            htmlTen
                                .selectFirst("span:contains(Адрес места нахождения:) + p")
                                ?.ownText()
                                ?.trim { it <= ' ' }
                                ?: ""
                        val factAdr = ""
                ...
                val delivPlace =
                    htmlTen.selectFirst("span:contains(Место поставки:) + p")?.ownText()?.trim { it <= ' ' }
                        ?: ""
                val delivTerm =
                    htmlTen.selectFirst("span:contains(Сроки поставки:) + p")?.ownText()?.trim { it <= ' ' }
                        ?: ""
                ...
                purobj1.forEach {
                    val name =
                        it.selectFirst("div.leftPart > p:eq(0)")?.ownText()?.trim { it <= ' ' }
                            ?: ""
                    val s = it.selectFirst("td:eq(1)")
                    val p = it.selectFirst("td:eq(0)")
                    val okei =
                        it.selectFirst("div.centerPart > div > p:eq(0)")?.ownText()?.trim { it <= ' ' }
                            ?: ""
                    val quantity =
                        it
                            .selectFirst("div.centerPart > div > p:eq(1)")
                            ?.ownText()
                            ?.replace(',', '.')
                            ?.trim { it <= ' ' }
                            ?: ""
                    val price =
                        it
                            .selectFirst("div.rightPart > div > p:eq(0)")
                            ?.ownText()
                            ?.replace(',', '.')
                            ?.trim { it <= ' ' }
                            ?: ""
                    val sum =
                        it
                            .selectFirst("div.rightPart > div > p:eq(1)")
                            ?.ownText()
                            ?.replace(',', '.')
                            ?.trim { it <= ' ' }
                            ?: ""
                    val fullOkpd =
                        it.selectFirst("div.leftPart a")?.ownText()?.trim { it <= ' ' }
                            ?: ""
                    val okpd2 = fullOkpd
                    val okpdName = ""
                    con
                        .prepareStatement(
                            "INSERT INTO ${Prefix}purchase_object SET id_lot = ?, id_customer = ?, name = ?, okei = ?, quantity_value = ?, customer_quantity_value = ?, price = ?, sum = ?, okpd2_code = ?, okpd_name = ?",
                        ).apply {
                            setInt(1, idLot)
                            setInt(2, idCustomer)
                            setString(3, name)
