...
import enterit.parsers.ParserLevel
...
class TenderLevel(val tn: SafmargT<String>, val driver: ChromeDriver) : TenderAbstract(), ITender {
    init {
        etpName = "Тендерная площадка Level Group"
        etpUrl = "https://etp.level.ru/"
    }

    companion object TypeFz {
        const val typeFz = 380
    }

    override fun parsing() {
        driver.get(tn.href)
        val wait = WebDriverWait(driver, ParserLevel.timeoutB)
        Thread.sleep(10000)
        try {
        ...
        var datePubT =
            driver.findElementWithoutException(By.xpath("//div[contains(preceding-sibling::div, 'Начало подтверждения участия')]"))?.text?.trim { it <= ' ' }
                ?: ""
        var pubDate = Date()
        ...
        DriverManager.getConnection(UrlConnect, UserDb, PassDb).use(fun(con: Connection) {
            ...
            var delivTerm5 =
                driver.findElementWithoutException(By.xpath("//div[contains(., 'Порядок формирования цены договора')]/following-sibling::div"))?.text?.trim { it <= ' ' }
                    ?: ""
            if (delivTerm5 != "") delivTerm5 = "Порядок формирования цены договора: $delivTerm5"
            var delivTerm6 =
                driver.findElementWithoutException(By.xpath("//div[contains(., 'Дополнительная информация')]/following-sibling::div"))?.text?.trim { it <= ' ' }
                    ?: ""
            if (delivTerm6 != "") delivTerm6 = "Дополнительная информация: $delivTerm6"
            var delivTerm = ""
            ...
            lots.forEachIndexed { index, lot ->
                ...
                purobj1.forEach {
                    val name2 = it.findElementWithoutException(By.xpath(".//td[3]"))?.text?.trim { it <= ' ' }
                        ?: ""
                    val name3 = it.findElementWithoutException(By.xpath(".//td[4]"))?.text?.trim { it <= ' ' }
                        ?: ""
                    val name4 = it.findElementWithoutException(By.xpath(".//td[7]"))?.text?.trim { it <= ' ' }
                        ?: ""
                    val name =
                        "${name2} | ${name3}  | ${name4}".trim { it <= ' ' }.trim { it <= '|' }.trim { it <= ' ' }
                    if (name == "Файл") {
                        return@forEach
                    }
