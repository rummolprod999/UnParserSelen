    ...
    override fun parsing() {
        ...
        try {
            wait.until(
                ExpectedConditions.visibilityOfElementLocated(
                    By.xpath("//div[contains(preceding-sibling::div, 'Начало подтверждения участия')]"),
                ),
            )
        } catch (e: Exception) {
        ...
        DriverManager.getConnection(UrlConnect, UserDb, PassDb).use(
            fun(con: Connection) {
                ...
                var IdOrganizer = 0
                val fullnameOrg =
                    driver.findElementWithoutException(By.xpath("//div[contains(preceding-sibling::div, 'Организатор')]"))?.text?.trim {
                        it <=
                            ' '
                    }
                        ?: ""
                ...
                var scoringDateT =
                    driver
                        .findElementWithoutException(
                            By.xpath("//div[contains(preceding-sibling::div, 'Дата подведения итогов')]"),
                        )?.text
                        ?.trim {
                            it <=
                                ' '
                ...
                var delivTerm5 =
                    driver
                        .findElementWithoutException(
                            By.xpath("//div[contains(., 'Порядок формирования цены договора')]/following-sibling::div"),
                        )?.text
                        ?.trim {
                            it <=
                                ' '
                ...
                var delivTerm6 =
                    driver
                        .findElementWithoutException(
                            By.xpath("//div[contains(., 'Дополнительная информация')]/following-sibling::div"),
                        )?.text
                        ?.trim {
                            it <=
                                ' '
                ...
                lots.forEachIndexed { index, lot ->
                    ...
                    val purobj1 =
                        lot.findElements(
                            By.xpath("//div[@class = 'k-grid-table-wrap' and @role = 'presentation']//tr[not(contains(., 'Деньги'))]"),
                        )
                    purobj1.forEach {
                        val name2 =
                        ...
                        val name3 =
                            it.findElementWithoutException(By.xpath(".//td[4]"))?.text?.trim { it <= ' ' }
                                ?: ""
                        val name4 =
                            it.findElementWithoutException(By.xpath(".//td[7]"))?.text?.trim { it <= ' ' }
                                ?: ""
                        val name =
