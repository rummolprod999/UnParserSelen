        ...
        DriverManager.getConnection(UrlConnect, UserDb, PassDb).use(
            fun(con: Connection) {
                ...
                if (urlOrgT != "") {
                    ...
                    if (fullnameOrg != "") {
                        val stmto =
                            con.prepareStatement("SELECT id_organizer FROM ${Prefix}organizer WHERE full_name = ?")
                        ...
                        if (rso.next()) {
                            ...
                            val postalAdr =
                                htmlOrg
                                    .selectFirst("label:containsOwn(Почтовый адрес) + div > div")
                                    ?.ownText()
                                    ?.trim { it <= ' ' }
                                    ?: ""
                            val factAdr =
                                htmlOrg
                                    .selectFirst("label:containsOwn(Юридический адрес) + div > div")
                                    ?.ownText()
                                    ?.trim { it <= ' ' }
                                    ?: ""
                            inn =
                                htmlOrg.selectFirst("label:containsOwn(ИНН) + div > div")?.ownText()?.trim { it <= ' ' }
                                    ?: ""
                            val kpp =
                ...
                var biddingDateT =
                    htmlLot
                        .selectFirst("label:containsOwn(Дата и время вскрытия конвертов) + div > div")
                        ?.ownText()
                        ?.trim { it <= ' ' }
                        ?: ""
                if (biddingDateT == "") {
                    biddingDateT = htmlLot
                        .selectFirst("div:containsOwn(Дата и время вскрытия конвертов) + div")
                        ?.ownText()
                        ?.trim { it <= ' ' }
                        ?: ""
                }
                ...
                val currency =
                    htmlLot.selectFirst("label:containsOwn(Валюта лота) + div > div")?.ownText()?.trim { it <= ' ' }
                        ?: ""
                var maxPriceT =
                    htmlLot.selectFirst("div:containsOwn(Начальная цена) + div")?.ownText()?.trim { it <= ' ' }
                        ?: ""
                if (maxPriceT == "") {
                    maxPriceT =
                        htmlLot
                ...
                val delivPlace =
                    htmlLot.selectFirst("label:containsOwn(Регион) + div > div")?.ownText()?.trim { it <= ' ' }
                        ?: ""
                val delivTerm =
                    htmlLot
