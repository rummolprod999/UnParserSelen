...
class TenderImpTorgov(val tn: TalanT) : TenderAbstract(), ITender {
    ...
    override fun parsing() {
        ...
        DriverManager.getConnection(UrlConnect, UserDb, PassDb).use(fun(con: Connection) {
            ...
            var biddingDateT = htmlLot.selectFirst("label:containsOwn(Дата вскрытия конвертов) + div > div")?.ownText()
                ?.trim { it <= ' ' }
                ?: ""
            if (biddingDateT == "") {
                biddingDateT =
                    htmlLot.selectFirst("div:containsOwn(Дата вскрытия конвертов) + div")?.ownText()?.trim { it <= ' ' }
                        ?: ""
            }
