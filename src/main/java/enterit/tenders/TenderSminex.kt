...
import enterit.parsers.ParserSminex
...
class TenderSminex(val tn: SafmargT<String>, val driver: ChromeDriver) : TenderAbstract(), ITender {
    init {
        etpName = "Тендерная система Sminex"
        etpUrl = "https://tender.sminex.com/"
    }

    companion object TypeFz {
        const val typeFz = 368
    }

    override fun parsing() {
        driver.get(tn.href)
        val wait = WebDriverWait(driver, ParserSminex.timeoutB)
        Thread.sleep(10000)
        try {
            wait.until(ExpectedConditions.visibilityOfElementLocated(By.xpath("//div[contains(preceding-sibling::div, 'Начало представления предложений') or contains(preceding-sibling::div, 'Начало приема квалификационных документов')  or contains(preceding-sibling::div, 'Начало подтверждения участия')]")))
        } catch (e: Exception) {
            logger("cannot find expected startDate", tn.href)
            return
        }
        var datePubT =
            driver.findElementWithoutException(By.xpath("//div[contains(preceding-sibling::div, 'Начало представления предложений') or contains(preceding-sibling::div, 'Начало приема квалификационных документов')  or contains(preceding-sibling::div, 'Начало подтверждения участия')]"))?.text?.trim { it <= ' ' }
                ?: ""
        var pubDate = Date()
        ...
        DriverManager.getConnection(UrlConnect, UserDb, PassDb).use(fun(con: Connection) {
            ...
            var IdOrganizer = 0
            val fullnameOrg =
                driver.findElementWithoutException(By.xpath("//div[contains(preceding-sibling::div, 'Организация')]//div[contains(@class, 'organization__info__title')]"))?.text?.trim { it <= ' ' }
                    ?: ""
            if (fullnameOrg != "") {
            ...
            var scoringDateT =
                driver.findElementWithoutException(By.xpath("//div[contains(preceding-sibling::div, 'Дата подведения итогов')]"))?.text?.trim { it <= ' ' }
                    ?: ""
            var scoringDate = Date()
            if (scoringDateT.contains("Вчера в")) {
                scoringDateT = scoringDateT.replace(
                    "Вчера в", SimpleDateFormat("dd.MM.yyyy").format(
                        Date.from(
                            LocalDate.now().minusDays(1).atStartOfDay(
                                ZoneId.systemDefault()
            ...
            scoringDateT = scoringDateT.getDataFromRegexp("""(\d{2}\.\d{2}\.\d{4}\s+\d{2}:\d{2})""")
            scoringDate = scoringDateT.getDateFromString(formatterGpn)
            ...
            var delivTerm2 =
                driver.findElementWithoutException(By.xpath("//div[contains(., 'Обеспечение суммы резерва качества')]/following-sibling::div/div"))?.text?.trim { it <= ' ' }
                    ?: ""
            if (delivTerm2 != "") delivTerm2 =
