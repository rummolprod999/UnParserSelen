        ...
        try {
            wait.until(
                ExpectedConditions.visibilityOfElementLocated(
                    By.xpath(
                        "//div[contains(preceding-sibling::div, 'Начало представления предложений') or contains(preceding-sibling::div, 'Начало приема квалификационных документов')  or contains(preceding-sibling::div, 'Начало подтверждения участия')]",
                    ),
                ),
            )
        } catch (e: Exception) {
            ...
            fun(con: Connection) {
                ...
                val fullnameOrg =
                    driver
                        .findElementWithoutException(
                            By.xpath(
                                "//div[contains(preceding-sibling::div, 'Организация')]//div[contains(@class, 'organization__info__title')]",
                            ),
                        )?.text
                        ?.trim {
                            it <=
