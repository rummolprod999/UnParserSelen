...
class TenderItMetal(val tn: SafmargT<String>, val driver: ChromeDriver, val wait: WebDriverWait) : TenderAbstract(),
    ITender {
    ...
    init {
        etpName = "Торгово-закупочная площадка горнодобывающих, металлургических и металлообрабатывающих предприятий ЛотЭксперт"
    ...
    override fun parsing() {
        ...
        DriverManager.getConnection(UrlConnect, UserDb, PassDb).use(fun(con: Connection) {
            ...
            lots.forEachIndexed { index, lot ->
                ...
                purobj1.forEach {
                    val test = it.findElementWithoutException(By.xpath(".//td[9]"))?.text?.trim { it <= ' ' }
                        ?: ""
                    if (true) {
