...
import enterit.tenders.TenderBrusnika
...
class ParserBrusnika : Iparser {
    ...
    override fun parser() {
        ...
        try {
            ...
            tendersS.forEach {
                try {
                    val t = TenderBrusnika(it, driver)
                    t.parsing()
                } catch (e: Exception) {
                    logger("error in TenderBrusnika.parsing()", e.stackTrace, e, it.href)
                }
            }
        } catch (e: Exception) {
            logger("Error in parser function", e.stackTrace, e)
