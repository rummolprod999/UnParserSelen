...
class ParserTander : Iparser {
    ...
    override fun parser() {
        ...
        try {
            ...
            val periods =
                driver.findElements(
                    By.xpath("//tr[@height = '20']/td[@class = 'xs-table-cell']//div[contains(self::node(), 'Период проведения')]"),
                )

            val titles =
                driver.findElements(
                    By.xpath(
                        "//tr[@height = '20']/td[@class = 'xs-table-cell']//div[contains(self::node(), 'Сбор коммерческих предложений')]",
                    ),
                )
    ...
    private fun parserList(
        periods: List<WebElement>,
        titles: List<WebElement>,
        hrefs: List<WebElement>,
    ) {
