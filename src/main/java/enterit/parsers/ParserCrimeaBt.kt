...
import enterit.tenders.TenderCrimeaBt
...
class ParserCrimeaBt : Iparser {
    ...
    private fun getUrlsFromStartPage() {
        ...
        val urls = html.select("h2:contains(Тендеры республики Крым по отраслям) + table ul li a")
    ...
    private fun parsingTender(el: Element, url: String) {
        ...
        if (datePub == Date(0L) || dateEnd == Date(0L)) run { logger("bad dates", urlTend, dateS); return }
        val tn = CrimeaBT(purNum, urlTend, purObj, datePub, dateEnd, pwName, price, currency)
        try {
            val t = TenderCrimeaBt(tn)
            t.parsing()
        } catch (e: Exception) {
