...
import enterit.tenders.TenderSimferop
...
class ParserSimferop : Iparser {
    ...
    companion object WebCl {
        const val BaseUrl = "https://zakupki-simferopol.rts-tender.ru/"
        const val timeoutB = 30L
        const val CountPage = 20
    }

    private val tendersS = mutableListOf<TenderSimferop>()
        ...
        try {
            ...
            tendersS.forEach {
                try {
                    ...
                    logger("error in TenderSimferop.parsing()", e.stackTrace, e, it.url)
                }
            }
        } catch (e: Exception) {
    ...
    private fun getListTenders(
        ...
        Thread.sleep(5000)
        wait.until(
            ExpectedConditions.visibilityOfElementLocated(By.xpath("//table[@id = 'jqGrid']/tbody/tr[not(@class = 'jqgfirstrow')][10]")),
        )
    ...
    private fun parserTender(el: WebElement) {
        ...
        val tt = TenderSimferop(status, purNum, purObj, nmck, datePub, dateEnd, url)
        tendersS.add(tt)
    }
}
