...
import enterit.tenders.TenderRostov
...
class ParserRostov : Iparser {
    ...
    companion object WebCl {
        const val BaseUrl = "https://rostovoblzmo.rts-tender.ru"
        const val timeoutB = 30L
        const val CountPage = 20
    }

    private val tendersS = mutableListOf<TenderRostov>()
        ...
        try {
            ...
            tendersS.forEach {
                try {
                    ...
                    logger("error in TenderRostov.parsing()", e.stackTrace, e, it.url)
                }
            }
        } catch (e: Exception) {
    ...
    private fun parserTender(el: WebElement) {
        ...
        val tt = TenderRostov(status, purNum, purObj, nmck, datePub, dateEnd, url)
        tendersS.add(tt)
    }
}
