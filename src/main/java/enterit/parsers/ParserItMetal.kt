...
import enterit.tenders.TenderItMetal
...
class ParserItMetal : Iparser {
    ...
    companion object WebCl {
        const val BaseUrl = "https://etp.metal-it.ru/trades?documents"
    ...
    override fun parser() {
        ...
        try {
            ...
            tendersS.forEach {
                try {
                    val t = TenderItMetal(it, driver, wait)
                    t.parsing()
                } catch (e: Exception) {
                    logger("error in TenderItMetal.parsing()", e.stackTrace, e, it.href)
                }
            }
        } catch (e: Exception) {
            logger("Error in parser function", e.stackTrace, e)
