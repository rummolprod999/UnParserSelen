...
import enterit.tenders.TenderSminex
...
class ParserSminex : Iparser {
    ...
    companion object WebCl {
        const val BaseUrl =
            "https://tender.sminex.com/trades"
    ...
    override fun parser() {
        ...
        try {
            ...
            tendersS.forEach {
                try {
                    val t = TenderSminex(it, driver)
                    t.parsing()
                } catch (e: Exception) {
                    logger("error in TenderSminex.parsing()", e.stackTrace, e, it.href)
                }
            }
        } catch (e: Exception) {
