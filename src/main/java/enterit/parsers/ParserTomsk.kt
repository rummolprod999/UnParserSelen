...
import enterit.tenders.TenderTomsk
...
class ParserTomsk : Iparser {
    ...
    companion object WebCl {
        const val BaseUrl = "https://region70.rts-tender.ru/"
        const val timeoutB = 30L
        const val CountPage = 20
    }

    private val tendersS = mutableListOf<TenderTomsk>()
        ...
        try {
            ...
            tendersS.forEach {
                try {
                    ...
                    logger("error in TenderTomsk.parsing()", e.stackTrace, e, it.url)
                }
            }
        } catch (e: Exception) {
    ...
    private fun parserTender(el: WebElement) {
        ...
        val tt = TenderTomsk(status, purNum, purObj, nmck, datePub, dateEnd, url)
        tendersS.add(tt)
    }
}
