...
private fun parserTender(
        ...
        datePubT =
            el
                .findElementWithoutException(
                    By.xpath(
                        "//table[@aria-describedby = 'grid_TenderGridViewModel_info']/tbody/tr[contains(@class, 'child')][$ind]//span[contains(., 'Начало приема заявок:')]/following-sibling::span",
                    ),
                )?.text
                ?.trim {
                    it <=
    ...
    if (placingWayName == "") {
        placingWayName =
            el
                .findElementWithoutException(
                    By.xpath(
                        "//table[@aria-describedby = 'grid_TenderGridViewModel_info']/tbody/tr[contains(@class, 'child')][$ind]//span[contains(., 'Способ проведения:')]/following-sibling::span",
                    ),
                )?.text
                ?.trim {
                    it <=
    ...
    var urlOrg =
        el.findElementWithoutException(By.xpath(".//td[8]/a"))?.getAttribute("href")?.trim { it <= ' ' }
            ?: ""
    if (urlOrg == "") {
        urlOrg =
            el
                .findElementWithoutException(
                    By.xpath(
                        "//table[@aria-describedby = 'grid_TenderGridViewModel_info']/tbody/tr[contains(@class, 'child')][$ind]//span[contains(., 'Организатор:')]/following-sibling::span/a",
                    ),
                )?.getAttribute("href")
                ?.trim { it <= ' ' }
                ?: ""
